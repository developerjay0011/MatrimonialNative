...
export const sendInterest = async (toUserId: string, message?: string) => {
    try {
        const response = await postData(INTEREST_ENDPOINTS.SEND_INTEREST, { data: { toUserId, message } });
...
export const getSentInterests = async () => {
    try {
        const response = await getData(INTEREST_ENDPOINTS.GET_SENT);
        return response.data;
    } catch (error) {
        throw error;
    }
...
export const getReceivedInterests = async () => {
    try {
        const response = await getData(INTEREST_ENDPOINTS.GET_RECEIVED);
        return response.data;
    } catch (error) {
        throw error;
    }
